    ...
    for year in range(START_YEAR, END_YEAR + 1):
        for month in range(1, 13):
            cur.execute(
                ...
                (year * 10000 + month * 100, year, month,
                 (month - 1) // 3 + 1, season(month)),
            )
            count += 1
    return count


def _insert_daily(cur) -> int:
    count = 0
    ...
    while d <= end:
        ...
        cur.execute(
            ...
            (tk, d, d.year, d.month, d.day,
             (d.month - 1) // 3 + 1, season(d.month),
             is_ramadan(d), is_eid_adha(d)),
        )
        d += timedelta(days=1)
        count += 1
    return count
